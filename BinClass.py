...
class VectorBins(Vector2d,Bins):
    """
    This class contains methods for binning Vector2d 
    objects."""


    def binthat2dVec(self):
        pass

class BinPlots(Vector2d):
    """
    This class contains methods for plotting Bin()
    objects."""
    ...
    def plot_comp_row(axs,bins):
        ...
        for col in range(Ncols):
            ...
            plot_prof(axs[col],bins[col][0])
        
    def plot_wndWundertow(wndres,stdrift,axs):
        """
        This function plots both components of 
        binned 2d results as wndres-stdrift.
        INPUTS:
            -wndres = binned results of the wind driven 
                    current model as Vector2d object.
            -stdrift = binned results of the stokes-drift 
                    profiles as Vector2d object.
        PLOTS:
            -Both components of the binned profiles on seperate axes.
        RETURNS:
            -Vector2d axs with self.i,self.j = ax_i,ax_j."""
        
        #     Nrows,Ncols = np.shape(wndres)
        plotting_icomp = True 
        while plotting_icomp:
            bins = wndres.i - stdrift.i # i undertow
            axrow = axs.i
            plot_comp_row(bins,axrow)
            plotting_icomp = False
        while not plotting_icomp: # plotting j comp
            bins = wndres.j - stdrift.j # j undertow
            axrow=axs.j
            plot_comp_row(bins,axrow)
            done_plotting = True
            plotting_icomp = done_plotting # end loop
        return axs